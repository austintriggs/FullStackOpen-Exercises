...
```mermaid
%%{init: { "sequence": { "wrap": true, "width":415 } } }%%
sequenceDiagram
    actor user
    participant browser
    participant server
    ...
    Note right of user: The JavaScript code updates the notes on the page using the DOM
    browser->>server: POST https://studies.cs.helsinki.fi/exampleapp/new_note_spa
    activate server
    server-->>browser: {"message":"note created"}
    deactivate server
    
    Note right of browser: The new note is sent to the server and the server sends a message in response
```
